...
/** @brief Snapshot of element interactions captured from the previous/current frame. */
struct InteractionSnapshot {
	/** @brief Clay element ids hovered during the snapshot. */
	std::vector<Clay_ElementId> hoveredElementIds;
	/** @brief Clay element ids pressed during the snapshot. */
    std::vector<Clay_ElementId> pressedElementIds;
	/** @brief Clay element ids held during the snapshot. */
    std::vector<Clay_ElementId> heldElementIds;
	/** @brief Clay element ids released during the snapshot. */
    std::vector<Clay_ElementId> releasedElementIds;

	/** @brief Return true if the list contains an element with the same Clay id. */
...
class UiManager;
/** @brief Convert a FlowUi element id string to a Clay element id. */
Clay_ElementId flowUiToClayElementId(UiManager& uiManager, std::string_view elementID);

/** @brief Return the previous interaction snapshot from a UI manager. */
const InteractionSnapshot& flowUiPreviousInteraction(const UiManager& uiManager);

/** @brief Push a constructed Clay element into the UI manager stack. */
