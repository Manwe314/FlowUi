...
This is not an FSEL-only feature. FSEL should provide excellent built-in metadata and previews, but every valid Flow element and every application using the core runtime must be able to participate. The developer system is therefore a library subsystem with FSEL-based tooling on top, not a property of the FSEL catalog.
...
This section is intentionally brief. It records what the repository has now and how well it matches the current storage, element, action, theme, and rendering systems.
...
- The retained 180 samples are private; tooling receives a latest-frame and aggregate view, not a navigable timeline.
...
There are also two correctness/integration gaps to fix before building more tooling:
...
## 2.2 Developer functionality: inspect, alter, and persist
...
## 2.3 Developer tooling: where the DX becomes real
...
| Build tooling from legacy dev elements? | Migrate to current element system and FSEL | Dogfoods supported APIs and removes duplication | Migration work precedes feature expansion |
...
└── DevToolHost            companion/docked/offline UI consumers
```
