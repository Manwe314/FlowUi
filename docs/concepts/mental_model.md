...
The managers are usually touched during this same construction phase. `FlowUi::FontManager` resolves a logical family and style into a concrete `FontId` for Clay text. `ImageManager`, `IconManager`, and `ViewPortManager` return logical, generation-checked `TextureRef` values. Raw Clay image commands stage their per-command payload through `UiManager::imageData()`. `InputFieldManager` is requested by custom input elements so text, caret, focus, and selection can persist across frames. `ShortcutManager` dispatches registered chords from frame input and uses scopes to decide which callbacks should run. `PopupManager` turns stable popup identities and placement requests into Clay floating configurations while carrying measurement and dismissal state between frames.
...
`app.drawFrame()` is the rendering phase. FlowUi takes the finalized command data and records Vulkan work for the current frame. Viewport callbacks record into managed offscreen targets, the UI renderer turns Clay commands into draw runs, logical texture handles use the current window's prepared descriptor bindings, and the swapchain image is presented through the window surface. This separation is intentional: `endFrame()` answers "what should be drawn?" while `drawFrame()` answers "submit it to the GPU and present it."
...
`UiManager` is the authoring surface for the current frame. It owns the Clay context, frame string arenas, texture reference storage, previous interaction snapshot, current and previous input snapshots, input field manager, shortcut manager, popup manager, clipboard bridge, cursor requests, and font resolution bridge. It is where user code turns Flow ids and runtime data into Clay-safe frame data.
...
`PopupManager` solves reusable floating behavior without imposing one widget vocabulary. Custom elements request parent, element, pointer, position, or viewport placement; the manager handles first-frame measurement, overflow correction, popup layers, and outside-press or Escape dismissal. The element remains responsible for its surface, children, and application-owned open state.
...
The managers are separated because each subsystem has different lifetime and frame rules. Fonts are long-lived atlas data. Images are app assets. Icons are lazily rasterized variants. Input fields are persistent text state. Shortcuts are dispatch tables. Popups retain short placement and dismissal sessions across frames. Viewports are offscreen frame resources. Keeping those concerns separate makes the runtime easier to understand and gives advanced users clear places to integrate custom behavior.
